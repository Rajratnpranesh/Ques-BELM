...
class Collator(object):
    ...
    def _get_positive_input_ids(self, data, question_tokens):
        ...
        if data['annotations'][0]['short_answers']:
            ...
            #print(words_to_tokens_index[start_position1-candidate_start],words_to_tokens_index[end_position1-candidate_start])
    ...
    def __call__(self, batch_idx):
        neg_num = 2
        batch_size = 2*len(batch_idx)+neg_num #two negative sample per batch
        ...
        for i, pos_idx in enumerate(batch_idx):
            ...
            #print(input_tokens)
            #print(input_ids)
            #print(len(input_tokens),len(input_ids))
            #print()
            batch_input_ids[i*2, :len(input_ids)] = input_ids
            batch_token_type_ids[i*2, :len(input_ids)] = [0 if k<=input_ids.index(3) else 1 for k in range(len(input_ids))]
            if annotations['short_answers']:
            ...
            input_tokens = ['[CLS]'] + question_tokens + ['[SEP]'] + answer_tokens + ['[SEP]']
            input_ids = self.tokenizer.convert_tokens_to_ids(input_tokens)
            batch_token_type_ids[i*2+1, :len(input_ids)] = [0 if k<=input_ids.index(3) else 1 for k in range(len(input_ids))]
        ...
        for i, neg_idx in enumerate(batch_idx[:neg_num]):
            ...
            input_tokens = ['[CLS]'] + question_tokens + ['[SEP]'] + answer_tokens + ['[SEP]']
            input_ids = self.tokenizer.convert_tokens_to_ids(input_tokens)
            batch_token_type_ids[idx, :len(input_ids)] = [0 if k<=input_ids.index(3) else 1 for k in range(len(input_ids))]
...
def main():
    ...
    model = AlbertForQuestionAnswering.from_pretrained('weights/epoch1/', config=config)
