...
class TFQADataset(Dataset):
    def __init__(self, id_list, neg_id_list):
    ...
    def __getitem__(self, index):
        return index # index only


class Collator(object):
    def __init__(self, id_list, neg_id_list, data_dict, neg_data_dict, new_token_dict, tokenizer, max_seq_len=384, max_question_len=64):
        self.id_list = id_list
        self.neg_id_list = neg_id_list
    ...
    def __call__(self, batch_idx):
        ...
        neg_num = 2
        batch_size = 2*len(batch_idx)+neg_num 
        ...
        for i, pos_idx in enumerate(batch_idx):
            ...
            if annotations['short_answers']:
                if start_position<0 or end_position<0: # if the groundtruth span not in the truncated data, ignore this positive data by setting labels to -1 
                    batch_y_start[i*2] = -1
                    batch_y_end[i*2] = -1
                    batch_y[i*2] = -1
                else:
        ...
        for i, neg_idx in enumerate(batch_idx[:neg_num]):
            ...
            input_tokens = ['[CLS]'] + question_tokens + ['[SEP]'] + answer_tokens + ['[SEP]']
            input_ids = self.tokenizer.convert_tokens_to_ids(input_tokens)
            batch_token_type_ids[idx, :len(input_ids)] = [0 if k<=input_ids.index(102) else 1 for k in range(len(input_ids))]
...
def random_sample_negative_candidates(candidate_index_list, prob_list):
    if len(prob_list) <= 0:
        return 0
    temp = np.random.random()
    value = 0.
    for index in range(len(prob_list)):
        value += prob_list[index]
        if value > temp:
            break
    return candidate_index_list[index]
...
def main():
    ...
    # prepare input
    # Load the distribution_dict for hard negative mining. The distribution_dict was splitted into four files due to memory issues.
    import pickle
    ...
    max_data = 9999999999 # for debugging

    id_list = [] # positive (long answer) document id list 
    neg_id_list = [] # negative document id list
    data_dict = {} # Stores the positive answer candidate and one randomly selected negative answer candidate for each positive document
    neg_data_dict = {} # Stores one randomly selected candidate for each negative document
    with open(json_dir) as f:
        for n, line in tqdm(enumerate(f)):
            ...
            if is_pos and len(data['long_answer_candidates'])>1: # discard positive documents with no negative candidate
                ...
                prob_list /= sum(prob_list) # make a prob distribution (sum to 1)
                # random sampling using the prob distribution
    ...
    # length of neg_id_list must be longer than id_list otherwise data generator will error. See the generator for more information.
    random.shuffle(neg_id_list) 
    ...
    batch_size = 3 # 3 (doc ids) x 2 (pos and neg pair) + 2 (neg from two other doc ids) = 8 data in a batch for each process/gpu
    ep = 0 # not in use
    ...
    model = BertForQuestionAnswering.from_pretrained(model_path, config=config) # replace this model_path to train 2nd, 3rd epochs...

    # add new tokens, these html tokens are from DATA STATISTICS https://github.com/google-research-datasets/natural-questions
    new_token_dict = {
                      '<P>':'qw1',
                      '<Table>':'qw2',
                      '<Tr>':'qw3',
    ...
    for j,(batch_input_ids, batch_attention_mask, batch_token_type_ids, batch_y_start, batch_y_end, batch_y) in enumerate(train_generator):
        ...
        # These class accuracy/loss accumulations may not be correct, just ignore them. I relied on the dev data for performance evaluation. 
